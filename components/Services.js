      ...
      <div className="w-full col-span-12 px-4 pb-8 md:px-12">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {services?.map((service, index) => (
            ...
            >
              <p className="whitespace-nowrap font-semibold">{service.service_name}</p>
              <p className="text-xs h-full">{service?.service_description} </p>
              <div className="flex py-3 text-xs">
                <p>Service Hours :</p>
                <p className="px-1 font-semibold">{service.service_duration} Min</p>
              </div>
              <div className="flex w-full items-center justify-between">
                <p className="text-xl font-bold text-primary-500">{service.services_charges} £</p>
                <button
